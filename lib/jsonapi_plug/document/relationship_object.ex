defmodule JSONAPIPlug.Document.RelationshipObject do
  ...
  defp deserialize_data(%{"data" => resource_identifier}) when is_map(resource_identifier),
    do: ResourceIdentifierObject.deserialize(resource_identifier)

  defp deserialize_data(%{"data" => resource_identifiers}) when is_list(resource_identifiers),
  ...
  defp deserialize_links(%{"links" => links}),
    do: Enum.into(links, %{}, fn {name, link} -> {name, LinkObject.deserialize(link)} end)
